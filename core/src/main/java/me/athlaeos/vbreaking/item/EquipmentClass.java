...
public enum EquipmentClass {
    ...
    EquipmentClass(String key, Material... matches) {
        ...
        this.matchingMaterials = new Tag<>() {
            private final NamespacedKey k = new NamespacedKey(ValhallaModulesBreaker.getInstance(), "tag_" + key);
    ...
    public static EquipmentClass getMatchingClass(Material type) {
        for (EquipmentClass tc : EquipmentClass.values()) {
            if (tc.getMatchingMaterials().contains(type)) return tc;
        }
        return null;
    }

    public static boolean isHandHeld(Material type) {
        EquipmentClass equipmentClass = getMatchingClass(type);
