...
import me.athlaeos.vbreaking.item.EquipmentClass;
...
public class ArmorSwitchListener implements Listener {
    ...
    private void updateArmor(Player who){
        ...
        if (update != null) update.refresh();
            ...
            update.runTaskTimer(ValhallaModulesBreaker.getInstance(), 1L, 1L);
        }
    ...
    public void onInventoryEquip(InventoryClickEvent e){
        ...
        if (e.getClickedInventory().getType() == InventoryType.PLAYER || e.getClickedInventory().getType() == InventoryType.CREATIVE){
            if (e.getSlotType() == InventoryType.SlotType.ARMOR){
                ...
                EquipmentClass type = EquipmentClass.getMatchingClass(clicked.getType());
                if (type == null || !EquipmentClass.isArmor(type)) return; // clicked item isn't armor, and so it can be assumed no equipment is equipped
    ...
    private static class DelayedArmorUpdate extends BukkitRunnable{
        ...
        public void run() {
            if (timer <= 0){
                ...
                BlockUtils.resetMinerCache(who.getUniqueId());
